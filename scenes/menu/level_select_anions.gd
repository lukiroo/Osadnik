...
## =========================================================================
## level_select_anions.gd – wybór poziomu anionów
## -------------------------------------------------------------------------
...
## Inicjalizacja ekranu: przyciski, overlay i blokady.
func _ready() -> void:
	...
	_bind_level_button("G4_Ex1", func(): _on_level_button_pressed(4, "EX1"))
	_bind_level_button("G4_Ex2", func(): _on_level_button_pressed(4, "EX2"))
	_bind_level_button("G56_Ex1", func(): _on_level_button_pressed(5, "EX1"))
	_bind_level_button("G56_Ex2", func(): _on_level_button_pressed(5, "EX2"))
...
## Kliknięcie w poziom:
## - dla grupy 2 od razu startuje poziom na Normalnym,
...
func _show_difficulty_panel(group_id: int, exercise_id: String) -> void:
	if group_id == 0:
		diff_label.text = "ANALIZA MIESZANINY ANIONÓW"
	else:
		var exercise_text := (
			"IDENTYFIKACJA POJEDYNCZYCH ANIONÓW"
			if exercise_id == "EX1"
			else "IDENTYFIKACJA MIESZANINY ANIONÓW"
		)
...
func _bind_level_button(node_name: String, callback: Callable) -> void:
	...
	if btn == null:
		push_error("Brak przycisku poziomu (aniony): %s" % node_name)
		return
...
func _refresh_level_locks() -> void:
	var level_button_map: Dictionary = {
		"G1_Ex1":  Settings.level_key_anions(1, "EXERCISE_SINGLE"),
		"G1_Ex2":  Settings.level_key_anions(1, "EXERCISE_MIX"),
		"G2_Ex1":  Settings.level_key_anions(2, "EXERCISE_SINGLE"),
		"G2_Ex2":  Settings.level_key_anions(2, "EXERCISE_MIX"),
		"G3_Ex1":  Settings.level_key_anions(3, "EXERCISE_SINGLE"),
		"G3_Ex2":  Settings.level_key_anions(3, "EXERCISE_MIX"),
		"G4_Ex1":  Settings.level_key_anions(4, "EXERCISE_SINGLE"),
		"G4_Ex2":  Settings.level_key_anions(4, "EXERCISE_MIX"),
		"G56_Ex1": Settings.level_key_anions(5, "EXERCISE_SINGLE"),
		"G56_Ex2": Settings.level_key_anions(5, "EXERCISE_MIX")
	}
	...
	for node_name in level_button_map.keys():
		...
		var unlocked := Settings.is_unlocked_anions(level_key)

		btn.disabled = not unlocked
		btn.modulate.a = 1.0 if unlocked else 0.5

		_set_level_button_caption(node_name)
		var stars := Settings.get_best_stars_anions(level_key)
		_update_level_button_stars(node_name, stars)

	# Egzamin anionowy – klucz "A_EXAM".
	var exam_key := "A_EXAM"
	var exam_unlocked := Settings.is_unlocked_anions(exam_key)
	...
	var exam_stars := Settings.get_best_stars_anions(exam_key)
